// Searchable data for projects, events, and team members

export const projects = [
  { 
    name: 'CaseAI', 
    description: 'Built by Ben Macklin (Engineering) and Ben Lewandowski (Commerce), using AI to create a tool that helps students practice case studies for consulting interviews.',
    imageUrl: 'https://www.qvibe.online/CaseAI.png',
    slug: 'projects'
  },
  { 
    name: 'Smart Money Confluence', 
    description: 'Built by Max Rainville (Commerce), he used AI tools like Cursor to create a working tool that helped him make money trading stocks.',
    imageUrl: 'https://www.qvibe.online/SmartMoney.png',
    slug: 'projects'
  },
  { 
    name: 'Recover', 
    description: 'Built by Greg Brenner (Kinesiology), Greg created an iOS app that helped users recover from injuries by tracking their progress and providing personalized exercises.',
    imageUrl: 'https://www.qvibe.online/Recover.png',
    slug: 'projects'
  }
];

export const events = [
  {
    name: 'Cursor x QVibe Hackathon',
    description: 'We hosted an exciting hackathon where students came together to build innovative AI-powered projects. They collaborated, learned, and showcased their skills in a competitive yet supportive environment.',
    date: '2025-11-09',
    imageUrl: '/CursorHackathon5.jpeg',
    images: [
      '/CursorHackathon5.jpeg', 
      '/CursorHackathon3.jpeg', 
      '/CursorHackathon4.jpeg',
      '/IMG_0748.jpg',
      '/IMG_0751.jpg',
      '/IMG_4315.jpg',
      '/IMG_4319.jpg',
      '/IMG_4334.jpg',
      '/IMG_4359.jpg',
      '/IMG_4362.jpg',
      '/IMG_4366.jpg',
      '/IMG_4380.jpg',
      '/IMG_4383.jpg'
    ]
  },
  {
    name: 'Personal Website Workshop',
    description: 'We taught hands-on web development techniques and tools. This workshop covered building personal websites from scratch, including design, development, and deployment. It was perfect for beginners and experienced developers alike.',
    date: '2025-11-21',
    imageUrl: '/IMG_0378.jpeg',
    images: ['/IMG_0378.jpeg', '/IMG_0383.jpeg', '/IMG_0390.jpeg', '/IMG_0399.jpeg', '/IMG_0415.jpeg', '/IMG_0427.jpeg']
  }
];

export const teamMembers = [
  { name: 'Ben Macklin', position: 'Presidents', slug: 'our-team' },
  { name: 'Ben Lewandowski', position: 'Presidents', slug: 'our-team' },
  { name: 'Ray Goldberg', position: 'Presidents', slug: 'our-team' },
  { name: 'Ned Booth', position: 'Advisors', slug: 'our-team' },
  { name: 'Meyer Eskin', position: 'Advisors', slug: 'our-team' },
  { name: 'Charlotte King', position: 'Chief of Staff', slug: 'our-team' },
  { name: 'Ella McConnell', position: 'Chief of Staff', slug: 'our-team' },
  { name: 'Thaila Adda', position: 'Marketing', slug: 'our-team' },
  { name: 'Sasha Mais', position: 'Marketing', slug: 'our-team' },
  { name: 'Matthew Bowman', position: 'Partnerships', slug: 'our-team' },
  { name: 'Jacob Shull', position: 'Partnerships', slug: 'our-team' },
  { name: 'Kayla Hakimzadeh', position: 'Partnerships', slug: 'our-team' },
  { name: 'Alex Wittstock', position: 'Events', slug: 'our-team' },
  { name: 'Greg Brenner', position: 'Events', slug: 'our-team' },
  { name: 'Daniel Ifergan', position: 'Events', slug: 'our-team' },
  { name: 'Tanry Wang', position: 'Logistics', slug: 'our-team' },
  { name: 'Julian Chan', position: 'Logistics', slug: 'our-team' },
  { name: 'Jessie Olujic', position: 'Outreach', slug: 'our-team' },
  { name: 'Chiara Kim', position: 'Outreach', slug: 'our-team' },
  { name: 'Camren Fawbert', position: 'Outreach', slug: 'our-team' },
  { name: 'Warren Marcotte', position: 'EDII', slug: 'our-team' },
  { name: 'Will Sheppard', position: 'Finance', slug: 'our-team' },
  { name: 'Max Rainville', position: 'Finance', slug: 'our-team' },
  { name: 'Patrick Buckley', position: 'Culture', slug: 'our-team' },
  { name: 'Jay Persaud', position: 'Culture', slug: 'our-team' },
  { name: 'Brodie Gilles', position: 'Culture', slug: 'our-team' },
  { name: 'Lauren Leslie', position: 'Culture', slug: 'our-team' },
  { name: 'Gracie Wright', position: 'Culture', slug: 'our-team' }
];


